...
import {
  ...
  SCALE,
  computeScores,
  ANSWER_KEY, // пока не используется, но можно оставить
  PILLARS,
  LEVEL_DETAILS,
  PILLAR_DETAILS,
} from "../testData";
...
const PILLAR_ORDER = ["Cognitive", "Emotional", "Behavioral", "Social"];

const PILLAR_SKILLS_ORDER = {
  Cognitive: ["Agency", "Cognitive Flexibility", "Situation Analysis"],
  Emotional: ["Emotion Regulation", "Stress Resilience", "Recovery"],
  Behavioral: ["Decision-Making", "Perseverance", "Goal-Directed Action"],
  Social: ["Support Network", "Assertiveness", "Resilience in Relationships"],
};
...
export default function TestPage() {
  ...
  const roomCode = (search.get("room") || "").trim() || null;
  const isEventMode = !!roomCode;
  ...
  const handleFinishTest = async (answersSnapshot = answers) => {
  setSubmitError("");
  ...
  try {
    ...
    if (isEventMode) {
      ...
      const resp = await fetch(`${API_BASE_URL}/event-results`, {
        method: "POST",
  ...
  const renderLevelBadge = (level, sum) => {
    return (
  ...
  // --- narrative по уровню ---
  const levelInfo = meta ? LEVEL_DETAILS[meta.ratelLevel] ?? null : null;
  ...
  const topPillarInfo =
    topPillarName && PILLAR_DETAILS
      ? PILLAR_DETAILS[topPillarName] ?? null
      : null;

  // --- ТОП-3 СКИЛЛОВ ПО ВСЕМ ПИЛЛАРАМ ---
  const topSkillsInfo = useMemo(() => {
    ...
    for (const pillarName of Object.keys(meta.skills)) {
      const skillsObj = meta.skills[pillarName];
      for (const [skillName, data] of Object.entries(skillsObj)) {
        flat.push({
          pillarName,
          skillName,
          sum: data.sum,
          ...
          <div className="testIntroCard">
            <div className="testIntroCard__left">
              ...
              <p className="pageSubtitle">
                Test mierzy Twoją odporność psychiczną w czterech kluczowych filarach: poznawczym, emocjonalnym, behawioralnym i społecznym. Zobaczysz, jak wypadasz na tle innych osób, które wypełniły test.
              </p>
              ...
              <p className="testIntroNote">
                Możesz w każdej chwili wrócić do testu, aby przejrzeć odpowiedzi lub wypełnić go ponownie.
              </p>
                  ...
                  >
                    ...
                    <g transform="translate(130,130)">
                      ...
                      {(() => {
                        ...
                        const cog =
                          meta.pillars[PILLARS.Cognitive]?.pct ?? 0;
                        const emo =
                          meta.pillars[PILLARS.Emotional]?.pct ?? 0;
                        const beh =
                          meta.pillars[PILLARS.Behavioral]?.pct ?? 0;
                        const soc =
                          meta.pillars[PILLARS.Social]?.pct ?? 0;

                        const toRadius = (pct) => (pct / 100) * BASE;
              ...
              <div className="pillarList">
                ...
                <ul>
                  {PILLAR_ORDER.map((pillarName) => {
                    const pdata = meta.pillars[pillarName];
            ...
            {topPillarInfo && (
              <div className="pillarNarrative">
                ...
                <ul className="pillarNarrative__skills">
                  {Object.entries(topPillarInfo.skills).map(
                    ([skillName, desc]) => (
                      <li key={skillName}>
              ...
              <div className="topSkillsBlock">
                ...
                {isBalanced ? (
                  <p className="topSkillsBlock__hint">
                    Twoje umiejętności są bardzo zrównoważone – nie ma jeszcze żadnej dziedziny, w której wyróżniałbyś się szczególnie.
                  ...
                  <p className="topSkillsBlock__hint">
                    Najwyższe wyniki surowe we wszystkich umiejętnościach we wszystkich czterech filarach.
                  </p>
                )}
                ...
                <div className="topSkillsGrid">
                  {topSkills.map((s) => {
                    const desc =
                      PILLAR_DETAILS?.[s.pillarName]?.skills?.[s.skillName];
                    return (
                      <div
                    ...
                    {PILLAR_ORDER.map((pillarName) => (
                      <tr key={pillarName}>
                        ...
                        {PILLAR_SKILLS_ORDER[pillarName].map((skillName) => {
                          const sData =
                            meta.skills?.[pillarName]?.[skillName] || null;
                          return (
                            <td key={skillName}>
            ...
            {isEventMode && !nicknameLocked && (
              <div className="nicknameBlock">
                ...
                <div className="nicknameHint">
                  Twoja nazwa użytkownika będzie widoczna dla wszystkich osób przebywających w tym pokoju wydarzeń
